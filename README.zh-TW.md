...
v0.1.0（[CHANGELOG](./CHANGELOG.md)）——160 條測試，其中 154 條完全離線（ffmpeg、ffprobe
...
**已在真實素材上端對端驗證**：一集 54 分鐘的 podcast，由 Apple Podcasts 連結解析、量測、
以 `speech` 處理，並經聽感確認——spread 10.0 → 5.8 LU。

**尚未涵蓋**：`segmented` 只在合成的階梯素材上驗證過，還沒跑過真實的漂移錄音。Linux 與
Windows 除了 CI 的單元測試之外未經實測。這個 skill 未在 Codex 下驗證過。假立體聲偵測是
以整檔為單位，所以本體 dual mono、片頭卻是真立體聲的節目會回報片頭那個較低的分離度——
安全但不精確。下載快取不會自動清理。
