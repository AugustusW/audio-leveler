...
- **程式裡不含判斷。** `measure.py` 只出數字，`apply.py` 只執行明確參數，決定寫在
  `SKILL.md` 交給宿主模型做。
- **模型讀的是數字，不是音訊。** 一整集的判斷只靠一份約 400–800 token 的契約，
  音訊本身完全不會送進模型。
...
這也讓模型那一段很便宜。音訊完全不會被送出去：模型只讀一份診斷契約，回一個
`--filter` 值。實測的契約大小：

| 素材長度 | windows | 契約 | 粗估 token |
|---|---|---|---|
| 10 分鐘 | 4 | 811 B | 約 270 |
| 54 分鐘 | 9 | 1.3 kB | 約 440 |
| 2 小時 | 20 | 2.4 kB | 約 815 |
| 4 小時 | 40 | 4.5 kB | 約 1500 |

統計窗上限是六分鐘，所以超過大約半小時之後，契約每六分鐘音訊多一筆。所有 ffmpeg
的工作——量測與產出——都是純 Python 驅動 ffmpeg，沒有模型參與。
